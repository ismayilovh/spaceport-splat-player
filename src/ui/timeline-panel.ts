    ...
    constructor(events: Events, tooltips: Tooltips, args = {}) {
        ...
        workArea.dom.addEventListener('pointerdown', (event: PointerEvent) => {
            if (!scrubbing && event.isPrimary) {
                ...
                events.fire('timeline.setFrame2', frameFromOffset(event.offsetX));
            }
        });
        ...
        events.on('timeline.frame2', (frame: number) => {
            moveCursor(frame);
        });
    ...
    constructor(events: Events, tooltips: Tooltips, args = {}) {
        ...
        const skip = (dir: 'forward' | 'back') => {
            ...
            if (orderedKeys.length > 0) {
                ...
                if (nextKey === -1) {
                    events.fire('timeline.setFrame2', orderedKeys[dir === 'back' ? l - 1 : 0].frame);
                } else {
                    events.fire('timeline.setFrame2', orderedKeys[dir === 'back' ? (nextKey + l - 1) % l : nextKey].frame);
                }
            } else {
                // if there are no keys, just to start of timeline or end
                if (dir === 'back') {
                    events.fire('timeline.setFrame2', 0);
                } else {
                    events.fire('timeline.setFrame2', events.invoke('timeline.frames') - 1);
                }
            }
        };
