...
const createReadSource = async (assetSource: AssetSource, start?: number, end?: number) => {
    ...
    if (start === undefined || end === undefined) {
        ...
        if (assetSource.contents instanceof File) {
            source = assetSource.contents.slice(start, end);
        } else if (assetSource.contents instanceof Response) {
            // Handle Response - you'll need to get the blob first
            const blob = await assetSource.contents.blob();
            source = blob.slice(start, end);
        }
    } else {
