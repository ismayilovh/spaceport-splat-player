...
const initFileHandler = (scene: Scene, events: Events, dropTarget: HTMLElement) => {
    ...
    // const importFile2 = async (response:ImportResponse , animationFrame: boolean) => {
    //     try {
    //         const model = await scene.assetLoader.load({
    ...
    //         return model;
    //     } catch (error) {
    //         await showLoadError(error.message ?? error,'tmp.ply');
    //     }
    // };

     const importFile2 = async (url:ImportUrl , animationFrame: boolean) => {
        try {
            const model = await scene.assetLoader.load({
            ...
            await showLoadError(error.message ?? error,url.filename);
        }
    };
    ...
    events.function('single-import', (response: ImportResponse, animationFrame = false) => {
        return importFile2(response, animationFrame);
    });
